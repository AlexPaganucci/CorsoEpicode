		...
		.then((data) => {
			...
			if (elenco.length > 0) {
				...
				elenco.map(function (element) {
					...
					<button type="button" onClick="modifica(${element.id})" class="btn btn-warning me-1 border border-dark"><i class="bi bi-pen-fill"></i></button>
...
function aggiungiModifica(x) {
	if (nome.value && cognome.value) {
		fetch(`http://localhost:3000/elenco/${x}`, {
			method: 'PUT',
			headers: {
				'Content-Type': 'application/json;charset=utf-8',
			},
